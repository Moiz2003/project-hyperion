...
export default function Input({
    ...
    const sharedClasses = cn(
        'w-full bg-[#000] border rounded-xl px-4 py-3 text-slate-300 font-inter text-sm',
    ...
    return (
        <div className="relative">
            {label && (
                <label
                    className={cn(
                        'absolute left-4 transition-all duration-200 pointer-events-none font-inter text-xs',
                        isActive
            ...
            />
            {error && (
                <p className="mt-1 text-[10px] font-inter text-red-400">{error}</p>
            )}
        </div>
    )
}
